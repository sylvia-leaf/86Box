...
enum
{
    ATIRAGE_IO_BASE_2EC = 0x02EC,    // Sparse I/O Base Address is 0x02ec
    ATIRAGE_IO_BASE_1CC = 0x01CC,    // Sparse I/O Base Address is 0x01cc
    ATIRAGE_IO_BASE_1C8 = 0x01C8,    // Sparse I/O Base Address is 0x01c8
    ATIRAGE_IO_BASE_INVALID
};
...
enum {
    MACH64_GT = 0,      /* 3D Rage */
    MACH64_GTB,         /* 3D Rage II */
    MACH64_LT,          /* 3D Rage LT */
    MACH64_GT2DVD,      /* 3D Rage II+ DVD*/
    MACH64_GT2C,        /* Rage IIC */
    MACH64_GTPRO,       /* 3D Rage Pro, Rage Pro Turbo */
    MACH64_LTPRO,       /* Rage LT Pro */
    MACH64_GMXL,        /* Rage XL */
    MACH64_GMXC,        /* Rage XC */
    MACH64_LMP          /* Rage Mobility-P */
};

#define ATIRAGE_FLAG_ONBOARD (1 << 19)
#define ATIRAGE_PCI_IOCONFIG 0x40        // "User Defined Configuration"
...
typedef struct atirage_t {
    ...
    uint32_t dst_bres_err; /* Aliased as lead_bres_dec in Rage */
    uint32_t dst_bres_inc; /* Aliased as lead_bres_dec in Rage */
    ...
    uint32_t bus_cntl; /* BUS_CNTL emulation is experimental */
    ...
    /* RAGE texture mapping, aliased to RAGE scaler */
    uint32_t tex_0_off;     /* Aliased to SCALE_Y_OFF */
    uint32_t tex_1_off;
    uint32_t tex_2_off;
    uint32_t tex_3_off; 
    uint32_t tex_4_off;
    uint32_t tex_5_off;
    uint32_t tex_6_off;
    uint32_t tex_7_off;     /* Aliased to SCALE_WIDTH */
    uint32_t tex_8_off;     /* Aliased to SCALE_HEIGHT */
    uint32_t tex_9_off;
    uint32_t tex_10_off;
    uint32_t s_y_inc;       /* Aliased to SCALE_Y_PITCH */
    uint32_t s_x_inc2;
    uint32_t s_y_inc2;
    uint32_t s_xy_inc2;
    ...
    /* RAGE colour/z/alpha interpolation, aliased to RAGE scaler */
    uint32_t red_x_inc;     /* Aliased to SCALE_X_INC */
    uint32_t red_y_inc;
    uint32_t red_start;     /* Aliased to SCALE_HACC */
    uint32_t green_x_inc;   /* Aliased to SCALE_Y_INC */
    uint32_t green_y_inc;
    uint32_t green_start;
    uint32_t blue_x_inc;    /* Aliased to SCALE_XUV_INC */
    uint32_t blue_y_inc;
    uint32_t blue_start;    /* Aliased to SCALE_UV_HACC */
    uint32_t z_x_inc;
    uint32_t z_y_inc;
    uint32_t z_start;
    ...
    struct {
        ...
        int      dst_x;         /* leading for trapezoids */
        int      dst_y;
        int      dst_x_start;
        int      dst_y_start;


        int      trail_x;       /* trailing for trapezoids */
        int      trail_err;
        int      trail_xinc;
...
extern video_timings_t timing_atirage;
...
enum {
    ...
    DST_X_TILE              = 0x00008,
    DST_Y_TILE              = 0x00010,
    DST_LAST_PEL            = 0x00020,
    DST_POLYGON_EN          = 0x00040,
    DST_24_ROT_EN           = 0x00080,
    DST_24_ROT              = 0x00700,
    DST_BRES_SIGN           = 0x00800,
    DST_POLYGON_RTEDGE_DIS  = 0x01000,
    TRAIL_X_DIR             = 0x02000,
    TRAP_FILL_DIR           = 0x04000,
...
void     atirage_overlay_draw(svga_t *svga, int displine);
void     atirage_queue(atirage_t *atirage, uint32_t addr, uint32_t val, uint32_t type);
...
void     atirage_fifo_thread(void *param);
void     atirage_wake_fifo_thread(atirage_t *atirage);
void     atirage_wait_fifo_idle(atirage_t *atirage);
...
extern atirage_t *reset_state[2];
