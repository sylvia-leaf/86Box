    ...
    switch (cpu_s->cpu_type) {
        ...
        case CPU_GENERICINTEL:
            if (!EAX) {
                ...
                EAX = CPUID;
                EBX = ECX = 0;
                EDX       = CPUID_FPU | CPUID_VME | CPUID_PSE | CPUID_TSC | CPUID_MSR | CPUID_PAE | CPUID_MCE | CPUID_CMPXCHG8B | CPUID_AMDSEP | CPUID_MMX | CPUID_MTRR | CPUID_PGE | CPUID_MCA | CPUID_SEP | CPUID_FXSR | CPUID_CMOV | CPUID_SSE | CPUID_SSE2 | CPUID_CLFLUSH | CPUID_NX;
            } else
                EAX = EBX = ECX = EDX = 0;
            break;
