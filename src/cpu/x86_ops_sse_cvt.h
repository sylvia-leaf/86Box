...
static int opCVTTPS2PI_mm_xmm_a16(uint32_t fetchdat)
{
    ...
    if (cpu_mod == 3)
    {
        cpu_state.MM[cpu_reg].l[0] = floor(XMM[cpu_rm].f[0]);
        cpu_state.MM[cpu_reg].l[1] = floor(XMM[cpu_rm].f[1]);
        CLOCK_CYCLES(1);
    }
    else
    {
...
static int opCVTTSS2SI_l_xmm_a16(uint32_t fetchdat)
{
    ...
    if (cpu_mod == 3)
    {
        setr32(cpu_reg, floor(XMM[cpu_rm].f[0]));
        CLOCK_CYCLES(1);
    }
    else
    {
...
static int opCVTPS2PI_mm_xmm_a16(uint32_t fetchdat)
{
    ...
    if (cpu_mod == 3)
    {
        cpu_state.MM[cpu_reg].l[0] = sse_round_to_int(XMM[cpu_rm].f[0]);
        cpu_state.MM[cpu_reg].l[1] = sse_round_to_int(XMM[cpu_rm].f[1]);
        CLOCK_CYCLES(1);
    }
    else
    {
        ...
        dst_real[0] = *(float*)&dst[0];
        dst_real[1] = *(float*)&dst[1];
        cpu_state.MM[cpu_reg].l[0] = sse_round_to_int(dst_real[0]);
        cpu_state.MM[cpu_reg].l[1] = sse_round_to_int(dst_real[1]);
        CLOCK_CYCLES(2);
    }
    return 0;
}
...
static int opCVTSS2SI_l_xmm_a16(uint32_t fetchdat)
{
    ...
    if (cpu_mod == 3)
    {
        setr32(cpu_reg, sse_round_to_int(XMM[cpu_rm].f[0]));
        CLOCK_CYCLES(1);
    }
    else
    {
        ...
        float dst_real;
        dst_real = *(float*)&dst;
        setr32(cpu_reg, sse_round_to_int(dst_real));
        CLOCK_CYCLES(2);
    }
    return 0;
}
