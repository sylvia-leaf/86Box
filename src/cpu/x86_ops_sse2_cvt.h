...
opCVTPI2PD_xmm_mm_a16(uint32_t fetchdat)
{
    ...
    XMM[cpu_reg].d[0] = src.l[0];
    XMM[cpu_reg].d[1] = src.l[1];
    fesetround(FE_TONEAREST);
    check_sse_exceptions_double(&XMM[cpu_reg].d[0]);
    check_sse_exceptions_double(&XMM[cpu_reg].d[1]);
    CLOCK_CYCLES(1);
    return 0;
}
...
opCVTTPD2PI_mm_xmm_a16(uint32_t fetchdat)
{
    ...
    SSE_GETSRC();
    dst->l[0] = trunc(XMM[cpu_rm].d[0]);
    dst->l[1] = trunc(XMM[cpu_rm].d[1]);
    MMX_SETEXP(cpu_reg);
    return 0;
}
...
opCVTPD2PS_mm_xmm_a16(uint32_t fetchdat)
{
    ...
    if (cpu_mod == 3) {
        ...
        XMM[cpu_reg].l[3] = 0;
        fesetround(FE_TONEAREST);
        check_sse_exceptions_float(&XMM[cpu_reg].f[0]);
        check_sse_exceptions_float(&XMM[cpu_reg].f[1]);
        CLOCK_CYCLES(1);
    } else {
        uint64_t dst[2];
...
opCVTDQ2PS_xmm_xmm_a16(uint32_t fetchdat)
{
    ...
    if (cpu_mod == 3) {
        ...
        check_sse_exceptions_float(&XMM[cpu_reg].f[0]);
        check_sse_exceptions_float(&XMM[cpu_reg].f[1]);
        check_sse_exceptions_float(&XMM[cpu_reg].f[2]);
        check_sse_exceptions_float(&XMM[cpu_reg].f[3]);
        CLOCK_CYCLES(1);
    } else {
        uint32_t dst[4];
