...
opSQRTPS_xmm_xmm_a16(uint32_t fetchdat)
{
    ...
    if (cpu_mod == 3) {
        ...
        //check_sse_execeptions_float(&cpu_state_high.XMM[cpu_reg].f[0]);
        //check_sse_execeptions_float(&cpu_state_high.XMM[cpu_reg].f[1]);
        //check_sse_execeptions_float(&cpu_state_high.XMM[cpu_reg].f[2]);
        //check_sse_execeptions_float(&cpu_state_high.XMM[cpu_reg].f[3]);
        CLOCK_CYCLES(1);
    } else {
        uint32_t src[4];
